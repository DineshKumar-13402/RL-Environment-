...
class RestaurantEnv:
    ...
    def _calculate_reward(self, action, prev_queue, prev_revenue, prev_satisfaction):
        if self.task_name == "task_1":
            # Normalized — queue of 80 = full penalty ← FIXED
            ...
            step_score = max(0.0, min(1.0, 1.0 - wait_penalty - overstaffing_penalty))
            return Task1Reward(
                score=step_score,
            ...
            revenue_reward = min(1.0, revenue_generated / 5000.0)  # ← FIXED normalization
            waste_penalty = min(0.3, self.simulator.waste_yesterday / 300.0)  # ← FIXED
            stockout_penalty = 0.1 if self.simulator.satisfaction < prev_satisfaction else 0.0
            step_score = max(0.0, min(1.0, revenue_reward * 0.6 - waste_penalty * 0.3 - stockout_penalty * 0.1 + 0.4))
            return Task2Reward(
                score=step_score,
            ...
            efficiency = max(0.0, 1.0 - (self.simulator.queue_size / 60.0))  # ← FIXED
